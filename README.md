...
- エンディングは全8種：登頂成功→下山完了、登り／下山それぞれの「力尽きた」、夢オチ、クマと冬眠、温泉から出られない、山小屋のバイトにハマる、登山をしない（観光日和）。
...
- `test/mountain-climb.test.mjs`: `mountain-climb.is` 固有のテスト。初期表示・安全ルートでの決定論的クリア・登頂成功/下山完了のMFM演出・1日1回ロック・統計的に発生するエンド（力尽きた／夢オチ／クマと冬眠／温泉・山小屋バイト）の出現を検証します。
