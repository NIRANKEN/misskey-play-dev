...
### `plays/mountain-climb.is` — 本日の登山チャレンジ！

3手のルート選択（登り2手＋下山1手）で山頂と無事な下山を目指すテキストアドベンチャーです。

- 結果は**日替わり固定（その日＆ユーザーごとの乱数シード `Math:gen_rng`）**で決まります。
- 今日の山タイプは4種類（各25%の完全均等配分）：
  1. **超低山・お散歩（25%）**: 天保山、日和山、円山など。どのルートを選んでも安全に登頂＆下山完了（完全制覇）できるボーナス日。
  2. **本格名山（25%）**: 富士山、立山、大山など。正解ルートを慎重に見極めれば登頂＆下山完了可能。
  3. **険峰（25%）**: 槍ヶ岳、剱岳、谷川岳など。登頂までは成功できるが、下山は日没・天候急変で全滅（登頂止まり）。
  4. **遭難必至の魔山（25%）**: 竜神山、阿蘇火口、バミューダ峰など。入山すると1手目で全滅確定。**初手の「登山をしない」を選んだ人が 🎉 大正解・神回避（完全勝利）！**
- スタート画面には「現地の様子」として、山の危険度やコンディションを示唆する**絶妙な匂わせヒント**が表示されます。
- エンディングは全27種：完全制覇、神回避、観光日和、登頂止まり、力尽きた3種（登山口/山頂直前/下山中）、通常ネタエンド15種（夢オチ、UFO、クマ冬眠、天狗修行、山ガチャ再会、野鳥観察、駅伝、SNSバズ、秘湯、山小屋バイト、鹿せんべい係、キノコ狩り、異世界転移、猫カフェ化、パラグライダー）、魔山専用ネタエンド5種（ブラジル貫通、湯釜スライム化、百鬼夜行フェス、鬼ヶ島討ち入り、超巨大かき氷職人）。
- 登頂成功時（中間）と下山完了/神回避時には、それぞれ華やかなMFMエフェクト（`$[tada ...]` / `$[x2 $[sparkle ...]]`）を表示します。
- 結果は**1日1回**です。`Mk:save` / `Mk:load` で当日の進行状況・結果を保存し、同じ日に再度開くと確定した結果（または続き）が表示されます。
...
3. タイトル・概要（summary）・公開範囲は、Play編集画面のフォームで別途設定してください。
   - タイトル例: `本日の登山チャレンジ！`
   - 概要例: `3手のルート選択で山頂と下山を目指すテキストアドベンチャー。遭難確定の魔山を「登山しない」で回避する直感も重要！（日替わり）`
4. 保存して「Play」ボタンから動作を確認してください。
